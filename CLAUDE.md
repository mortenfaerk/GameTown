...
283 tests, mostly HTTP-level against the real app booted through `WebApplicationFactory` on a throwaway SQLite database created from `Database/sqlite/*.sql` — the same files the application embeds and applies on a fresh install, so DDL/model drift fails here.
...
- **`ArchiveDirectoryFailureTests` is about the response BODY, not its status.** An archive directory the service cannot write to always answered 500; what it did not carry was a reason, so the SPA could only render `ApiResult`'s generic "Request failed (500)." and the cause was legible nowhere but the server's journal. This is the failure a Proxmox mountpoint added over `GameFilesPath` after installation actually produced. It also pins the other half of the decision: a read-only archive directory must still serve every download in the library, which is why the writability probe is on `GetWritableGameDirectoryAsync` and not on `GetGameDirectoryAsync`.
...
- **`LanMatchingTests` is mostly about what must NOT match.** It pins the normaliser behind automatic LAN-suggestion matching — case (in C#, because SQLite's NOCASE folds ASCII only), punctuation, roman numerals, a leading article. The negatives are the point: `Portal` ≢ `Portal 2`, `Left 4 Dead` ≢ `Left 4 Dead 2`, `Warcraft 3` ≢ `Warcraft 3: Warlock (Custom Map)`. A normaliser that strips trailing numbers or parentheticals passes every positive and then links the wrong game — and unlike a missed match, which someone fixes by hand in seconds, a wrong auto-link is never looked at again.
- **The banding half of `LanMatchingTests` pins the thresholds that decide what a person is *shown*.** `MatchConfidenceBands` splits a ranked list into strong/ambiguous/weak so the wishlist can render three kinds of row differently. The cases that matter are the ones where a high score is wrong: `Age of Empires 2` reaches **Age of Empires IV** at 0.825 — the roman fold puts `age-of-empires-2` one edit from `age-of-empires-4`, while the correct `...-2-definitive-edition` is penalised for length — so a plain score threshold pre-ticks the wrong game and one bulk click pushes it to Discord. What separates the correct rows from the wrong ones is the *margin* to the runner-up (0.500–0.886 against 0.000–0.269), which is why the bar is a margin and why it is 0.35. The flat-field rule at the other end has the same shape: `swat 4` leads at 0.414 with Battlefield 4 at 0.392 behind it and SWAT 4 is not in the library at all, so a mediocre leader in a crowd is the matcher having no opinion. Banding is presentation only and must never widen the automatic path, which is still normalised equality and nothing else.
- **`LanIntegrationTests` carries this suite's only stubbed dependency, and the fake is the test.** Everything else here runs against the real thing; outbound providers are covered through their *refusals*, which never reach the network. The LAN bot cannot be: every rule worth pinning is on the success path. So `FakeLanBot` reproduces three behaviours **verified against the live bot rather than read off its OpenAPI document** — a PUT replaces the catalogue entry's single title; it binds only suggestions that are currently *unbound*; and it never unbinds or re-points anything. Get those wrong and every test passes against a bot that does not exist. The thrash test syncs three times on purpose: a re-push would leave the catalogue title flip-flopping between two names forever, and one sync passes either way.
...
`Database/sqlite/01_schema.sql` is the **frozen baseline (version 1)**. Do not edit it to change the schema. Add a numbered migration in `Database/sqlite/migrations/` instead (`003_what_it_does.sql`), then re-scaffold. The schema is currently at **version 8**.
...
### LAN bot (Discord suggestions)
- The LAN this appliance is shown at runs a Discord bot where players suggest games. `LanBotClient` / `LanSuggestionService` mirror its `/suggestions` feed, match names against the library, and push each match back so the suggestion carries a link to the game. `LanSyncWorker` is **the only hosted service in this codebase** (the abandoned-upload sweep in `Program.cs` is a one-shot inline call, not a worker): it creates a scope per tick, re-reads its interval from settings each loop so a change takes effect without a restart, does nothing at all when unconfigured, and waits before its first run.
- **GameTown does not read matches from the bot — it *causes* them, since Catalogue API v1.4.0 in one of two ways.** The bot's catalogue is keyed by a GUID the *client* chooses. `matchId` is always the **GameTown game's own `Guid`** — confirmed with the bot's author, not merely inferred from the field being called `gameTownMatchId`. A game push (`PUT /games/{matchId}`, `LanBotClient.UpsertGameAsync`) writes the entry's title/url/boxArtUrl and, as a side effect, adopts any currently-unmatched suggestion whose name case-insensitively equals the title — but the reliable way to link a *specific* suggestion is a direct bind (`PUT /suggestions/{id}/match`, `LanBotClient.MatchSuggestionAsync`), which works regardless of the suggestion's raw text and regardless of what it is currently bound to. The title sent on a push is GameTown's own canonical one — binding no longer depends on it matching a suggestion's raw text, so there is no reason to send anything else.
- **A game push never overwrites an existing match; a direct bind always can.** A push only adopts suggestions that are currently unbound (reporting `suggestionsBound: 0` for one that already is). A direct bind (`PUT .../match`) re-points an already-bound suggestion unconditionally, last-writer-wins — that is what lets `LinkAsync` claim a suggestion the crew bound by hand in Discord, or one bound to a different GameTown game. A direct unbind (`DELETE .../match`) clears just that suggestion's binding, leaving the catalogue entry and every other suggestion pointing at it untouched — `UnlinkAsync` uses this in every case now, with no asymmetry between a GameTown-created entry and an adopted one. Several suggestions can still share one game, since bindings at the far end are additive, and **nothing is ever re-pushed once bound** — each suggestion's own `RemoteMatchId == null` filter enforces that.
- **`AutoMatchBlocked` must survive the bulk path.** Unlinking sets it so the matcher stops deciding for that row; a ranked row carries it, and `Preselect` is computed on the server as `strong && !AutoMatchBlocked` precisely so a pre-ticked proposal cannot let one press of "Link selected" undo a decision somebody made by hand. Candidates are still offered — they may want to link it again — but never ticked.
- **The wishlist count lives in `LanCountState` on the client**, shared by the sidebar link and the screen's own tab badge. They used to disagree: `LanNavLink` read once on initialise and never again, so setting a suggestion aside moved one badge and not the other. It also decided whether to render by pulling the *entire* suggestion list whenever the unmatched count was zero — so the healthy steady state downloaded every suggestion on every page load to answer one boolean. `LanCountContract.Total` answers it instead.
- A row therefore carries two different claims and they must not be collapsed: `GameId` is what **GameTown** worked out (many-to-one; drives the badge, the `?lan=` filter and the wishlist) and `RemoteMatchId` is what the **bot** actually has. The bot is the authority on the second, and every sync reconciles to it rather than re-asserting local state.
- **The wishlist is three queues, not one list.** `GET /lan/suggestions/ranked` ranks the whole unmatched queue against the library in one pass and bands each row (`MatchConfidenceBands`), so the screen can tell the rows with one obvious answer from the rows nothing in the library matches without opening each one to find out. On a real 78-row queue that was 4 strong, 29 ambiguous and **45 with no plausible candidate at all** — and the old screen offered all 45 a "Find match" button that opened ten wrong answers. Weak rows therefore ship **no candidates on the wire**; the rest are trimmed to those within half the leader's score, at most five. Nothing links on a single click: a choice is staged and an explicit button commits, because a link is an outbound push that is awkward to undo. `POST /lan/link-many` and `/dismiss-many` do the bulk actions, sequenced on the server because the bot is rate-limited and a loop in the browser lives in a page that can be closed halfway through.
- **`LibraryGamePicker` is the way out when the ranking misses, and it is load-bearing.** `Rank` returns ten candidates and abbreviations are what people type in Discord: `CS2` ranked **Rust** first and never offered Counter-Strike 2, `DRG` never offered Deep Rock Galactic, and both games are in the library. Without a search there was nothing a person could do but link the wrong game or give up. It searches `/GTGames/search` — the shelf's own endpoint — not the metadata provider.
- **Contributors can run a sync**, not just admins: refreshing your own to-do list is not an integration action, and a contributor already makes outbound calls whenever they link or unlink. Only reading the sync *status* is Admin, because it reports on configuration. `LanSyncState.TryBeginRun` gates overlapping runs — two syncs at once would both decide a suggestion is unbound and both PUT for it — and the loser gets reason `"busy"`, which is a normal outcome rather than a failure.
- A failed *pull* must never reach reconciliation: an unreachable bot returns an empty list, which is indistinguishable from every suggestion having been withdrawn, and reconciliation would faithfully release the whole library's bindings on the strength of a network blip.
- Settings (`LanBotBaseUrl`, `LanBotApiKey`, `LanBotSyncIntervalMinutes`, `PublicBaseUrl`) are read **per call**, like every other setting. The `lanbot` HTTP client deliberately does **not** use `ImageFetcher.BuildHandler` — see SECURITY-NOTES.md risk 10 before changing that. Suggestion and event names are player-typed Discord text: plain text only, never `MarkupString` (risk 11).
- **`HCPBOT-API-ASKS.md` records the asks that shaped this design and what landed in Catalogue API v1.4.0.** The `"bound-elsewhere"` path and the `BoundElsewhere` flag it once described are gone — a direct bind resolves that case rather than merely reporting it. Read the file before assuming a limit it documents still holds; `?unbound=`/`?afterId=` filters and a `source` filter on `GET /games` also landed but are deliberately not used (see the file for why).
- `PublicBaseUrl` has a real consumer now: every LAN bot push sends it as the `url` (a deep link to the game) and, when the game has a cover, `boxArtUrl` on `UpsertGameRequest`. The SPA still resolves its own address from wherever it was loaded, so this setting is purely for what links in from outside — but it is no longer speculative. The `X-GameTown-Base-Url` header this used to ride along on is gone, replaced by the real field.
