...
- **GameTown does not read matches from the bot — it *causes* them.** The bot's catalogue is keyed by a GUID the *client* chooses, and it binds a suggestion to the entry whose `title` equals the suggestion's name. So `matchId` is always the **GameTown game's own `Guid`** — confirmed with the bot's author, not merely inferred from the field being called `gameTownMatchId` — and the title pushed is the **suggestion's text verbatim** — tidying it into the library's canonical title breaks the very binding the call exists to create.
- **Three behaviours of the bot were verified against the live one and are not in its OpenAPI document.** A PUT replaces the entry's single title; it binds only suggestions that are currently **unbound**, reporting `suggestionsBound: 0` otherwise; and it never unbinds or re-points anything — only `DELETE` removes bindings. Consequences: several suggestions can share one game, a suggestion the crew already bound cannot be claimed by GameTown at all (the screen says so rather than showing a link that does not exist), and **nothing is ever re-pushed** — a repeat push would leave the catalogue title oscillating between two names forever.
...
- **Several compromises here are waiting on the bot's API, not on us.** `HCPBOT-API-ASKS.md` records what was asked for, why, and *what changes in this codebase when each one lands* — the `"bound-elsewhere"` path and the `BoundElsewhere` flag exist only because a suggestion cannot currently be bound directly, and would be deleted rather than extended. Read it before building around one of these limits.
- `PublicBaseUrl` has no functional consumer inside GameTown today — the SPA resolves its own address from wherever it was loaded. It exists for things *outside* this install that want to link back in: the LAN screen renders the exact deep link per matched game so an operator can hand it over, and it rides along as an `X-GameTown-Base-Url` header the bot is free to start honouring.
